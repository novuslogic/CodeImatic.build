...
Const
  ...
  (* NT-based only	The file system directory that corresponds to the nonlocalized Startup program group for all users. *)
  CSIDL_COMMON_APPDATA = 35;
  (* The file system directory containing application data for all users. *)
  CSIDL_COMMON_DESKTOPDIRECTORY = 25;
  (* NT-based only	The file system directory that contains files and folders that appear on the desktop for all users. *)
  CSIDL_COMMON_DOCUMENTS = 46;
  (* The file system directory that contains documents that are common to all users. *)
  CSIDL_COMMON_FAVORITES = 31;
  (* NT-based only	The file system directory that serves as a common repository for favorite items common to all users. *)
  ...
  (* NT-based only	The file system directory that contains the directories for the common program groups that appear on the Start menu for all users. *)
  CSIDL_COMMON_STARTMENU = 22;
  (* NT-based only	The file system directory that contains the programs and folders that appear on the Start menu for all users. *)
  CSIDL_COMMON_STARTUP = 24;
  (* NT-based only	The file system directory that contains the programs that appear in the Startup folder for all users. *)
  CSIDL_COMMON_TEMPLATES = 45;
  (* NT-based only	The file system directory that contains the templates that are available to all users. *)
  ...
  CSIDL_DESKTOP = 0;
  (* The virtual folder representing the Windows desktop, the root of the shell namespace. *)
  ...
  CSIDL_PERSONAL = 5;
  (* The file system directory used to physically store a user's common repository of documents. (From shell version 6.0 onwards, CSIDL_PERSONAL is equivalent to CSIDL_MYDOCUMENTS, which is a virtual folder.) *)
  ...
  (* A folder for 32-bit components that are shared across applications on 64-bit systems. *)
  CSIDL_PROGRAMS = 2;
